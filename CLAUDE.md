...
This is a **COR24-native project**. All implementation code must run
on the COR24 ecosystem. Allowed languages:

- **PL/SW** — primary systems language (VM, runtime)
- **SNOBOL4** — text processing, parsing, compiler frontends
- **COR24 assembly** — inline via MACRODEF GEN or standalone .s files

**Not allowed**: Python, Rust, C, JavaScript, or any host-side
language for implementation code. The `tools/lam_asm.py` assembler
is a legacy development aid and should eventually be replaced with
a SNOBOL4 or PL/SW equivalent.

This policy exists because the project dogfoods the COR24 toolchain.
Every implementation artifact should compile and run on COR24.
